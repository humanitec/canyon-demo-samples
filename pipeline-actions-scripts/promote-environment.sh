...
github_workflow_params:
  ...
  inputs:
    ...
    source_env_id: \${{ inputs.source_env_id }}
    target_env_id: \${{ inputs.target_env_id }}
    org_id: \${{ pipeline.org.id }}
    run-id: \${{ pipeline.run.id }}
  url: https://github.com/humanitec/canyon-demo-samples/blob/main/.github/workflows/promote-environment.yaml
